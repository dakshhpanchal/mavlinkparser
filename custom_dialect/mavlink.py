...
MAVLINK_MSG_ID_SENSOR_DATA = 50000
MAVLINK_MSG_ID_GPS_DATA = 50001
...
class MAVLink_sensor_data_message(MAVLink_message):
    ...
    msgname = "SENSOR_DATA"
    fieldnames = ["timestamp", "temperature", "pressure", "humidity", "sensor_id", "battery_voltage"]
    ordered_fieldnames = ["timestamp", "temperature", "pressure", "humidity", "battery_voltage", "sensor_id"]
    fieldtypes = ["uint64_t", "float", "float", "float", "uint8_t", "float"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {}
    fieldunits_by_name: Dict[str, str] = {}
    ...
    def __init__(self, timestamp: int, temperature: float, pressure: float, humidity: float, sensor_id: int, battery_voltage: float):
    ...
    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.timestamp, self.temperature, self.pressure, self.humidity, self.battery_voltage, self.sensor_id), force_mavlink1=force_mavlink1)
...
class MAVLink_gps_data_message(MAVLink_message):
    """
    Custom GPS message
    """

    id = MAVLINK_MSG_ID_GPS_DATA
    msgname = "GPS_DATA"
    fieldnames = ["time", "latitude", "longitude", "altitude", "satellites", "fix_type"]
    ordered_fieldnames = ["time", "latitude", "longitude", "altitude", "satellites", "fix_type"]
    fieldtypes = ["uint64_t", "int32_t", "int32_t", "int32_t", "uint8_t", "uint8_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {}
    fieldunits_by_name: Dict[str, str] = {}
    ...
    def __init__(self, time: int, latitude: int, longitude: int, altitude: int, satellites: int, fix_type: int):
        MAVLink_message.__init__(self, MAVLink_gps_data_message.id, MAVLink_gps_data_message.msgname)
        self._fieldnames = MAVLink_gps_data_message.fieldnames
        self._instance_field = MAVLink_gps_data_message.instance_field
        self._instance_offset = MAVLink_gps_data_message.instance_offset
        self.time = time
    ...
    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.time, self.latitude, self.longitude, self.altitude, self.satellites, self.fix_type), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_gps_data_message, "name", mavlink_msg_deprecated_name_property())


mavlink_map: Dict[int, Type[MAVLink_message]] = {
    MAVLINK_MSG_ID_SENSOR_DATA: MAVLink_sensor_data_message,
    MAVLINK_MSG_ID_GPS_DATA: MAVLink_gps_data_message,
}
...
class MAVLink(object):
    ...
    def sensor_data_encode(self, timestamp: int, temperature: float, pressure: float, humidity: float, sensor_id: int, battery_voltage: float) -> MAVLink_sensor_data_message:
        """
        Custom sensor message

        timestamp                 : Timestamp (us) (type:uint64_t)
        temperature               : Temperature (C) (type:float)
        pressure                  : Pressure (hPa) (type:float)
        humidity                  : Humidity (%) (type:float)
        sensor_id                 : Sensor ID (type:uint8_t)
        battery_voltage           : Battery voltage (V) (type:float)

        """
        return MAVLink_sensor_data_message(timestamp, temperature, pressure, humidity, sensor_id, battery_voltage)

    def sensor_data_send(self, timestamp: int, temperature: float, pressure: float, humidity: float, sensor_id: int, battery_voltage: float, force_mavlink1: bool = False) -> None:
        """
        Custom sensor message
        ...
        """
        self.send(self.sensor_data_encode(timestamp, temperature, pressure, humidity, sensor_id, battery_voltage), force_mavlink1=force_mavlink1)

    def gps_data_encode(self, time: int, latitude: int, longitude: int, altitude: int, satellites: int, fix_type: int) -> MAVLink_gps_data_message:
        """
        Custom GPS message

        time                      : Timestamp (us) (type:uint64_t)
        latitude                  : Latitude * 1e7 (type:int32_t)
        longitude                 : Longitude * 1e7 (type:int32_t)
        altitude                  : Altitude (mm) (type:int32_t)
        satellites                : Satellite count (type:uint8_t)
        fix_type                  : Fix type (type:uint8_t)

        """
        return MAVLink_gps_data_message(time, latitude, longitude, altitude, satellites, fix_type)

    def gps_data_send(self, time: int, latitude: int, longitude: int, altitude: int, satellites: int, fix_type: int, force_mavlink1: bool = False) -> None:
        """
        Custom GPS message
        ...
        """
        self.send(self.gps_data_encode(time, latitude, longitude, altitude, satellites, fix_type), force_mavlink1=force_mavlink1)
